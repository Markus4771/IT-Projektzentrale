...
cp -a "$ROOT_DIR/app" "$ROOT_DIR/templates" "$ROOT_DIR/static" \
    "$ROOT_DIR/nginx" "$ROOT_DIR/systemd" \
    "$ROOT_DIR/requirements.txt" "$ROOT_DIR/version.txt" \
    "$ROOT_DIR/README.md" "$ROOT_DIR/CHANGELOG.md" \
    "$ROOT_DIR/CHATGPT_PROJEKTKONTEXT.md" "$ROOT_DIR/NEUER_CHAT.md" \
    "$ROOT_DIR/install.sh" \
    "$PACKAGE_ROOT/opt/it-projektzentrale/"

rm -rf "$PACKAGE_ROOT/opt/it-projektzentrale/app/__pycache__"

install -m 0644 "$ROOT_DIR/nginx/it-projektzentrale.conf" \
