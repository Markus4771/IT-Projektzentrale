...
def test_user_management_module_is_wired_into_current_release():
    ...
    runtime_module = (root / "app/v140.py").read_text(encoding="utf-8")
    service = (root / "systemd/it-projektzentrale.service").read_text(encoding="utf-8")

    assert "ROLES =" in user_module
    assert "from app.v130 import app" in runtime_module
    assert "app.v140:app" in service
    assert (root / "version.txt").read_text(encoding="utf-8").strip() == "1.4.0"
