...
def test_user_management_module_is_wired_into_current_release():
    ...
    release_module = (root / "app/v200.py").read_text(encoding="utf-8")
    service = (root / "systemd/it-projektzentrale.service").read_text(encoding="utf-8")

    assert "ROLES =" in user_module
    assert "from app.v190 import app" in release_module
    assert "app.v200:app" in service
    assert (root / "version.txt").read_text(encoding="utf-8").strip() == "2.0.0"
...
def test_navigation_links_platform_areas():
    ...
    for target in ("/admin/users", "/app-store", "/plugins", "/servers", "/infrastructure", "/maintenance", "/installation"):
